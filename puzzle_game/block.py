...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        for i in range(len(self.children)):
            if self.children[i].colour is not None:
    ...
    def smash(self) -> bool:
        ...
        if not self.smashable():
            ...
            for _ in range(4):
                ...
                board = Block(self.position, self._child_size(),
                              COLOUR_LIST[idx], self.level + 1, self.max_depth)
    ...
    def rotate(self, direction: int) -> bool:
        ...
        if len(self.children) == 0:
            ...
            for children in self.children:
                if children.colour is None:
                    children.rotate(direction)
            if direction == 1:
    ...
    def combine(self) -> bool:
        ...
        if self.level != self.max_depth - 1 or len(self.children) == 0:
            ...
            count = {}
            for children in self.children:
                if children.colour not in count:
                    count[children.colour] = 1
                else:
                    count[children.colour] += 1
            max_occ = max(count.values())
            color_lst = [color for color in count if count[color] == max_occ]
            if len(color_lst) == 1:
                self.colour = color_lst[0]
                self.children.clear()
                return True
            else:
                return False
    ...
    def create_copy(self) -> Block:
        ...
        if len(self.children) == 0:
            ...
            block = Block(self.position, self.size, self.colour, self.level,
                          self.max_depth)
            for children in self.children:
                block.children.append(children.create_copy())
            return block
