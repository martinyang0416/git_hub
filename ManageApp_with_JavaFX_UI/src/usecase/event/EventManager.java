...
public class EventManager implements EventManage{
	...
	private EventExtract         eventExtract;
	...
	public void initialize(PeopleManage pm, TaskManage tm, MessagingManage mm, FriendsManage fm, ScoreManage sm){
		...
		eventChange = new EventChange(this, tm, mm, fm);
		eventCreate = new EventCreate(this, tm, mm, fm, sm, eventConvert);
		eventExtract = new EventExtract(this, pm);
	...
	public List<PersonDTO> getSpeakers(UUID event){
		return eventExtract.getSpeakers(event);
	}
	
	@Override
	public PersonDTO getOrganizer(UUID event){
		return eventExtract.getOrganizer(event);
	}
	...
	@Override
	public boolean isStarted(UUID event){
		return isStarted(get(event));
	}
