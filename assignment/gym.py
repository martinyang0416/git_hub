...
class Instructor:
    ...
    def can_teach(self, workout_class: WorkoutClass) -> bool:
        ...
        for item in workout_class.get_required_certificates():
            if item not in self._certificates:
                return False
        return True
...
class Gym:
    ...
    def add_workout_class(self, workout_class: WorkoutClass) -> bool:
        ...
        if workout_class.get_name() in self._workouts:
            return False
        else:
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        if time_point not in list(self._schedule.keys()):
            self._schedule[time_point] = {}
        for instructor in self._schedule[time_point].values():
            if self._instructors[instr_id] in instructor:
                return False
        if room_name in self._schedule[time_point] or \
                self._instructors[instr_id].can_teach(self._workouts
                                                      [workout_name]) is False:
            return False
        else:
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        for item2 in list(self._schedule[time_point].values()):
            if client in item2[-1]:
                return False
        for key in list(self._schedule[time_point].keys()):
            if self._workouts[workout_name] in self._schedule[time_point][key]:
                if len(self._schedule[time_point][key][-1]) < self._rooms[key]:
                    self._schedule[time_point][key][-1].append(client)
                    return True
        return False
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        acc = []
        for key in self._schedule.items():
            if key[0] == time_point:
                for value in key[1].items():
                    acc.append((value[1][0].name, value[1][1].get_name(),
                                value[0]))
        return acc
    ...
    def instructor_hours(self, time1: datetime, time2: datetime) -> \
        ...
        acc, time_acc = {}, []
        for time_point in list(self._schedule.keys()):
            if time1 <= time_point <= time2:
                time_acc.append(time_point)
        for instr_id in list(self._instructors.keys()):
            if instr_id not in acc:
                acc[instr_id] = 0
        for time in time_acc:
            for item in list(self._schedule[time].values()):
                if item[0].get_id() in acc:
                    acc[item[0].get_id()] += 1
        return acc
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        time_table, acc = self.instructor_hours(time1, time2), []
        for inst_id, time in list(time_table.items()):
            money = base_rate * time + BONUS_RATE * \
                    self._instructors[inst_id].get_num_certificates() * time
            acc.append((inst_id, self._instructors[inst_id].name, time, money))
        acc.sort()
        return acc
