...
def windows(lst: List[Any], n: int) -> List[List[Any]]:
    ...
    for i in range(len(lst) - (n - 1)):
        ...
        for j in range(i, i + n):
            sub_win.append(lst[j])
        window.append(sub_win)
    return window
...
class AlphaGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        stu_lst = slice_list(sort_students(course.students, 'name'),
                             self.group_size)
        grouping = Grouping()
        for item in stu_lst:
            group = Group(item)
...
class RandomGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        """
        stu_lst = []
        while len(stu_lst) != len(course.students):
            random_choice = random.choice(course.students)
            if random_choice not in stu_lst:
                stu_lst.append(random_choice)
        stu_lst = slice_list(stu_lst, self.group_size)
        grouping = Grouping()
        for item in stu_lst:
            group = Group(item)
            grouping.add_group(group)
...
class GreedyGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        grouping = Grouping()
        curr_lst = []
        for index in range(len(course.get_students())):
            curr_lst.append(course.get_students()[index])
        while len(curr_lst) != 0:
            new_lst = [curr_lst.pop(0)]
            while len(new_lst) != self.group_size and len(curr_lst) != 0:
                score_lst = [survey.score_students(new_lst + [curr_lst[i]]) for
                             i in range(len(curr_lst))]
                max_index = score_lst.index(max(score_lst))
                new_lst.append(curr_lst.pop(max_index))
            grouping.add_group(Group(new_lst))
        return grouping
...
class WindowGrouper(Grouper):
    ...
    def make_grouping(self, course: Course, survey: Survey) -> Grouping:
        ...
        grouping = Grouping()
        curr_lst = []
        for i in range(len(course.get_students())):
            curr_lst.append(course.get_students()[i])
        index = 0
        while len(curr_lst) > self.group_size:
            win_lst = windows(curr_lst, self.group_size)
            win_lst.append(win_lst[0])
            if survey.score_students(win_lst[index]) >= survey.score_students(
                    win_lst[index+1]):
                grouping.add_group(Group(win_lst[index]))
                for student in win_lst[index]:
                    curr_lst.remove(student)
            else:
                index += 1
        grouping.add_group(Group(curr_lst))
        return grouping
