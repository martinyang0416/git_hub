...
class MultipleChoiceQuestion(Question):
    ...
    === Private Attributes ===
    _options: possible answers of this question
    ...
    def __str__(self) -> str:
        ...
        return template.format(self.text, str(self._options))
...
class NumericQuestion(Question):
    ...
    === Private Attribute ===
    min_: the minimum possible integer answer of this question
    max_: the maximum possible integer answer of this question
    ...
    def __str__(self) -> str:
        ...
        template = 'Question {},' \
                   'Enter an integer between {} to {} inclusive.'
        return template.format(self.text, self._min, self._max)
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        return 1.0 - (difference / (abs(self._max - self._min)))
...
class CheckboxQuestion(Question):
    ...
    === Private Attribute ===
    _options: the list of possible answers of this question
    ...
    def __str__(self) -> str:
        ...
        template = 'Question: {},' \
                   'Please choose all that you applied {}.'
        return template.format(self.text, str(self._options))
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        if not isinstance(answer.content, list):
            ...
            return False
        elif len(answer.content) != len(set(answer.content)):
            return False
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        acc = []
        same = []
        for item1 in answer1.content:
            if item1 not in acc:
                acc.append(item1)
        for item2 in answer2.content:
            if item2 not in acc:
                acc.append(item2)
        for item3 in answer1.content:
            for item4 in answer2.content:
                if item3 == item4 and item3 not in same:
                    same.append(item3)
        return len(same) / len(acc)
...
class Survey:
    ...
    def __str__(self) -> str:
        ...
        """
        string = ''
        for ques in self._questions.values():
            string += ques.__str__() + '\n'
        return string
    ...
    def score_students(self, students: List[Student]) -> float:
        ...
        if len(self._questions) == 0:
            return 0.0
        score = 0
        ques_lst = self.get_questions()
        for ques in ques_lst:
            criterion = self._get_criterion(ques)
            weight = self._get_weight(ques)
            ans_lst = []
            for student in students:
                ans_lst.append(student.get_answer(ques))
            try:
                qlt = criterion.score_answers(ques, ans_lst)
                ques_score = qlt * weight
