...
class Student:
    ...
    == Private Attributes ==
    _answer: A dictionary that stores that all answers,
        the key is the id of the question,
        and the value is the Answer to corresponding question.
    ...
    def has_answer(self, question: Question) -> bool:
        ...
        if question.id in self._answer:
            return question.validate_answer(self._answer[question.id])
        else:
            return False
    ...
    def get_answer(self, question: Question) -> Optional[Answer]:
        ...
        if question.id in self._answer:
            return self._answer[question.id]
        else:
            return None
...
class Course:
    ...
    def enroll_students(self, students: List[Student]) -> None:
        ...
        for student in students:
            if student.id not in id_lst and student.name.strip() != '':
                self.students.append(student)
                id_lst.append(student.id)
            else:
                self.students.clear()
                return None
        return None
    ...
    def all_answered(self, survey: Survey) -> bool:
        ...
        """
        valid_ans = True
        ques_lst = survey.get_questions()
        for student in self.students:
            for ques in ques_lst:
                if student.get_answer(ques) is None:
                    valid_ans = False
                elif not ques.validate_answer(student.get_answer(ques)):
                    valid_ans = False
        return valid_ans
