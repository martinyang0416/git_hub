...
class HomogeneousCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        if len(answers) == 1:
            return 1.0
        for index1 in range(len(answers) - 1):
            for index2 in range(index1 + 1, len(answers)):
                acc.append(question.get_similarity(answers[index1],
                                                   answers[index2]))
...
class HeterogeneousCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for answer in answers:
            if not answer.is_valid(question):
                raise InvalidAnswerError
        homo_criteria = HomogeneousCriterion()
        return 1.0 - homo_criteria.score_answers(question, answers)
...
class LonelyMemberCriterion(Criterion):
    ...
    def score_answers(self, question: Question, answers: List[Answer]) -> float:
        ...
        for answer in answers:
            if not answer.is_valid(question):
                raise InvalidAnswerError
            ans_lst.append(answer.content)
        for ans in answers:
            if ans_lst.count(ans.content) <= 1:
                score = False
        if score:
            return 1.0
        else:
